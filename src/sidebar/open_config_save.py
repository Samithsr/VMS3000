...
class SaveConfigDialog:
    """
    Save Configuration File dialog — VMS 3000 SCADA theme.
    
    Opens a file dialog to save .rcs configuration files.
    ...
    def show(self, default_filename="Configuration"):
        ...
        self._saved_file = filedialog.asksaveasfilename(
            title="VM3000 SOFTWARE FILES",
            parent=self._parent,
            defaultextension=".rcs",
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("VMS 3000 - Save Configuration Test")
    ...
    def on_save():
        dialog = SaveConfigDialog(root, fonts)
        file_path = dialog.show()
        if file_path:
            result_label.config(text=f"Saved: {file_path.split('/')[-1]}")
        else:
    ...
    result_label = tk.Label(
        root,
        text="Click button to save configuration file",
        font=fonts["sm"],
        bg=T["win_bg"],
        fg=T["text"],
    )
